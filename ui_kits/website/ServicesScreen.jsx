const { Card, Icon, Badge, Tabs, SectionHeading, Button, Breadcrumb } = window.MedicalAllianceDesignSystem_32f8e4;
  ...
  return (
    <Page>
      ...
      <Container style={{ paddingTop: 'var(--space-8)', paddingBottom: 'var(--section-y)' }}>
        <Tabs value={tab} onChange={setTab} items={Object.keys(DETAIL).map((k) => ({ value: k, label: DETAIL[k].title.split(' and ')[0] }))} />
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 'var(--space-12)', marginTop: 'var(--space-10)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-3)', marginTop: 'var(--space-2)' }}>
              <Button onClick={() => onNavigate('contact')}>Discuss this service</Button>
