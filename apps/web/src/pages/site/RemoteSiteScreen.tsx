...
import { Alert, Badge, Breadcrumb, Button, Card, DataTable, Icon, ProgressMeter, SectionHeading } from '../../components';
import { Container } from '../../layouts/site-chrome';

/** Migrated from ui_kits/website/RemoteSiteScreen.jsx — copy and layout unchanged. */
...
type SiteRow = { site: string; cover: string; head: string; status: string; tone: 'success' | 'warning' | 'info' };
...
export function RemoteSiteScreen() {
    ...
    <>
      <section className="ma-ink" style={{ background: 'var(--ma-maroon-900)', color: 'var(--text-primary)' }}>
        <Container style={{ paddingTop: 'var(--space-12)', paddingBottom: 'var(--space-16)' }}>
          <Breadcrumb items={[{ label: 'Home', href: '/' }, { label: 'Services', href: '/services' }, 'Remote sites']} style={{ marginBottom: 'var(--space-6)' }} />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 'var(--space-12)', alignItems: 'end' }}>
            <div>
      ...
      <Container style={{ paddingTop: 'var(--section-y-tight)', paddingBottom: 'var(--section-y-tight)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 'var(--space-5)', marginTop: 'var(--space-8)' }}>
          {STEPS.map((s, i) => (
            <Card key={s.t}>
      ...
      <section style={{ background: 'var(--surface-page-alt)', borderBlock: '1px solid var(--border-subtle)', padding: 'var(--section-y-tight) 0' }}>
        <Container style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 'var(--space-12)', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
            <Alert tone="brand" title="Mobilisation in 21 days" icon={<Icon name="calendar-check" size={18} />}
              actions={<Button size="sm" onClick={() => navigate('/contact')}>Start an assessment</Button>}>
