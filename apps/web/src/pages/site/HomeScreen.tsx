...
import { Button, Card, Icon, SectionHeading, StatTile } from '../../components';
import { Container, PhotoSlot } from '../../layouts/site-chrome';

/**
 * Migrated from ui_kits/website/HomeScreen.jsx. Markup, styling and copy are
 * unchanged; only navigation (useNavigate instead of an onNavigate prop) and the
 * logo asset path differ.
 */
    ...
    <>
      ...
      <section className="ma-ink" style={{ background: 'var(--ma-maroon-950)', color: 'var(--text-primary)', position: 'relative', overflow: 'hidden' }}>
        ...
        <Container style={{ position: 'relative', paddingTop: 'var(--space-24)', paddingBottom: 'var(--space-24)' }}>
          <div style={{ maxWidth: '22ch', display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-3)', marginTop: 'var(--space-2)', flexWrap: 'wrap' }}>
              <Button size="lg" onClick={() => navigate('/contact')} iconEnd={<Icon name="arrow-right" size={18} />}>Request a site assessment</Button>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,.10)' }}>
          <Container style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 'var(--space-6)', paddingTop: 'var(--space-6)', paddingBottom: 'var(--space-6)' }}>
      ...
      <section style={{ padding: 'var(--section-y) 0' }}>
        <Container>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--space-5)', marginTop: 'var(--space-10)' }}>
            {SERVICES.map((s) => (
              <Card key={s.title} interactive onClick={() => navigate('/services')}>
      ...
      <section style={{ background: 'var(--surface-page-alt)', borderBlock: '1px solid var(--border-subtle)', padding: 'var(--section-y) 0' }}>
        <Container style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 'var(--space-16)', alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
            ...
            <div><Button variant="secondary" onClick={() => navigate('/remote-sites')} iconEnd={<Icon name="arrow-right" size={16} />}>How site cover works</Button></div>
          </div>
        ...
        <Container>
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
              <Button size="lg" variant="ink" onClick={() => navigate('/contact')}>Request a consultation</Button>
