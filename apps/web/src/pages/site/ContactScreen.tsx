...
import { Badge, Breadcrumb, Button, Card, Checkbox, Icon, SectionHeading, SelectField, TextField, Toast } from '../../components';
import { Container } from '../../layouts/site-chrome';

/**
 * Migrated from ui_kits/website/ContactScreen.jsx — copy and layout unchanged.
 *
 * The form still does preventDefault + local toast exactly as the prototype did.
 * Wiring it to POST /api/contact is a Phase 2 task (Frontend Work → "Forms wired
 * to real endpoints"), deliberately out of Phase 1 scope, so nothing here
 * pretends to send a message it cannot send.
 */
export function ContactScreen() {
  ...
  return (
    <Container style={{ paddingTop: 'var(--space-10)', paddingBottom: 'var(--section-y)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 'var(--space-16)', alignItems: 'start' }}>
        <div>
          ...
          <form onSubmit={(e) => { e.preventDefault(); setSent(true); }}
            style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 'var(--space-5)', marginTop: 'var(--space-8)' }}>
